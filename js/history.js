...
// ========================================
// RENDU DE L'HISTORIQUE (séparé du fetch)
// Peut être appelé depuis le state local (optimistic) ou depuis l'API
// ========================================
function renderHistory(items, stats) {
  ...
  const sorted = [...items].sort((a, b) => (b.row_number || 0) - (a.row_number || 0));

  // Afficher l'historique (sans onclick inline pour éviter XSS)
  liste.innerHTML = sorted.map(r => {
...
`;
  ...
  // Mettre à jour l'eau et les barres de nutrition
  updateEauSection(stats);
  updateTotalFromData(items, stats);
}

// ========================================
// CHARGEMENT DEPUIS L'API (fetch + réconciliation)
// silent=true : pas d'indicateur de chargement (réconciliation en arrière-plan)
// ========================================
async function loadHistory(silent) {
  ...
  // Afficher un état de chargement seulement si pas en mode silencieux
  if (!silent) {
    liste.innerHTML = `
      <div class="empty-state">
        <div class="empty-icon">⏳</div>
  ...
  try {
    ...
    // Mettre à jour le state local (réconciliation)
    NutriState.loadFromAPI({ items, stats });

    // Rendre l'UI avec les données du serveur
    renderHistory(items, stats);

  } catch (error) {
    console.error('Erreur chargement historique:', error);

    // En mode silencieux, ne pas écraser l'affichage existant
    if (!silent) {
